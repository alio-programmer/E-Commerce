...
const Loginc = () => {
    ...
    const schema = Yup.object().shape({
        ...
        password: Yup.string().min(6, "password is too small").max(14, "password is too big").required(),
    });
        ...
        <div className='w-full h-full'>
            <div className='w-full h-full flex justify-center items-center placeholder:align-middle'>
                <Formik initialValues={initialValues} onSubmit={CallloginApi} validationSchema={schema} validateOnMount>
                    <Form className=' w-1/2 h-5/6 p-3 flex flex-col items-center rounded-2xl justify-center border-black border-solid border-2 shadow-2xl'
                        method="get"
                        ...
                        <div className='p-5 flex flex-col justify-center items-center '>
                            ...
                            <button type='submit' className='bg-blue-500 m-3 text-white p-2 rounded-md cursor-pointer hover:scale-105 shadow-xl'>Login</button>
                        </div>
                    </Form>
                </Formik>
            </div>
